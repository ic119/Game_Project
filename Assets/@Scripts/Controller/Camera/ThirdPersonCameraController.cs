...
namespace JJORY.Controller
{
    ...
    public class ThirdPersonCameraController : MonoBehaviour
    {
        ...
        #region Method
        /// <summary>
        /// ThirdPersonFollow 컴포넌트 찾기 (Unity 6 Cinemachine)
        /// </summary>
        private void FindThirdPersonFollowComponent()
        {
            if (cinemachineCamera == null) return;

            // Unity 6의 CinemachineCamera는 여러 컴포넌트를 가질 수 있음
            // ThirdPersonFollowCamera 또는 유사한 컴포넌트를 찾음
            Component[] components = cinemachineCamera.GetComponents<Component>();
            foreach (var comp in components)
            {
                string typeName = comp.GetType().Name;
                if (typeName.Contains("ThirdPerson") || typeName.Contains("Follow"))
                {
                    thirdPersonFollowComponent = comp;
                    Utils.CreateLogMessage<ThirdPersonCameraController>($"Found component: {typeName}");
        ...
        {
            // 여러 가능한 필드명 시도
            string[] possibleFieldNames = { "Distance", "DefaultDistance", "CameraDistance", "m_Distance" };
            ...
            foreach (string fieldName in possibleFieldNames)
            {
                ...
                {
                    distanceField = field;
                    Utils.CreateLogMessage<ThirdPersonCameraController>($"Found Distance field: {fieldName}");
                    break;
                }
            }
        }
